...
func (s *Service) VideoLikeAction(userId string, videoId string, action string) (int, string) {
	if action == "1" {
		err := s.db.VideoLikeCreate(userId, videoId)
		if err != nil {
			return consts.CodeDBCreateError, "LikeAction LikeCreate error"
		}
		err = s.db.VideoLikeCountUp(videoId)
		if err != nil {
	...
	if action == "2" {
		err := s.db.VideoLikeDelete(userId, videoId)
		if err != nil {
			log.Println(err)
			return consts.CodeDBDeleteError, "VideoLikeAction LikeDelete error"
		}
		err = s.db.VideoLikeCountDown(videoId)
		if err != nil {
			log.Println(err)
...
func (s *Service) CommentLikeAction(userId string, commentId string, action string) (int, string) {
	if action == "1" {
		err := s.db.CommentLikeCreate(userId, commentId)
		if err != nil {
			log.Println("CommentLikeAction LikeCreate error", err)
			return consts.CodeDBCreateError, "CommentLikeAction CommentCreate error"
		}
		err = s.db.CommentLikeCountUp(commentId)
		if err != nil {
	...
	if action == "2" {
		err := s.db.CommentLikeDelete(userId, commentId)
		if err != nil {
		...
		err = s.db.CommentLikeCountDown(commentId)
		if err != nil {
...
func (s *Service) LikeList(userId string, pageNum string, pageSize string) (int, string, []dto.Video, bool) {
...
func (s *Service) CommentPublish(videoId string, userId string, content string) (int, string) {
...
func (s *Service) CommentList(videoId string, pageSize string, pageNum string) (int, string, []dto.Comment, bool) {
	pageNumInt, err := strconv.Atoi(pageNum)
	if err != nil {
...
func (s *Service) CommentDelete(commentId string, videoId string, userId string) (int, string) {
