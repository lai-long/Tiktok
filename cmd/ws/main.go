...
func main() {
	...
	if err := logger.InitLogger(cfg.Log.Level, cfg.Log.Format, cfg.Log.Development, "ws", cfg.Log.Path); err != nil {
		logger.Fatal("初始化日志错误", zap.Error(err))
	}
	logger.Info("logger initialized", logger.WithServiceName("ws"), zap.String("level", cfg.Log.Level), zap.String("format", cfg.Log.Format))
