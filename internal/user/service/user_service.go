...
func (s *UserRepo) Login(ctx context.Context, userName, password, mfaCode string) (int32, *user.UserInfo, string, string, error) {
	...
	code, err := s.MfaConfirm(ctx, mfaCode, userInfo.ID)
	if err != nil {
...
func (s *UserRepo) GenerateMfa(ctx context.Context, username string, userId string) (string, string, int32, error) {
	...
	secret := key.Secret()
	err = s.userDb.SaveMfaSecret(ctx, secret, userId)
	if err != nil {
...
func (s *UserRepo) MfaBindByCode(ctx context.Context, code string, userId string) (int32, error) {
	defer utils.TrackTime(ctx, "MfaBindByCode")()
	secret, err := s.userDb.GetMfaSecret(ctx, userId)
	if err != nil {
	...
	err = s.userDb.MfaBindUpdate(ctx, userId)
	if err != nil {
...
func (s *UserRepo) MfaBindBySecret(ctx context.Context, secret string, userId string) (int32, error) {
	defer utils.TrackTime(ctx, "MfaBindBySecret")()
	dbSecret, err := s.userDb.GetMfaSecret(ctx, userId)
	if err != nil {
...
func (s *UserRepo) MfaConfirm(ctx context.Context, mfaCode string, userID string) (int32, error) {
